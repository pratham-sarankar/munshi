...
void main() {
  group('Average Daily Calculation Tests', () {
    test(
      'Average calculation should only count days up to today for future periods',
      () {
        // Get today's date
        ...
        final expectedDays =
            effectiveEndDate.difference(currentMonth.startDate).inDays + 1;
        ...
        final fullMonthDays =
            currentMonth.endDate.difference(currentMonth.startDate).inDays + 1;
    ...
    test('Average calculation should use full period for past periods', () {
      ...
      final expectedDays =
          effectiveEndDate.difference(lastMonthPeriod.startDate).inDays + 1;
      final fullPeriodDays =
          lastMonthPeriod.endDate.difference(lastMonthPeriod.startDate).inDays +
          1;
    ...
    test('Average calculation should handle daily periods correctly', () {
      ...
      final expectedDays =
          effectiveEndDate.difference(todayPeriod.startDate).inDays + 1;
    ...
    test('Average calculation should handle weekly periods correctly', () {
      ...
      final effectiveEndDate = currentWeek.endDate.isAfter(today)
          ? today
          : currentWeek.endDate;
      final expectedDays =
          effectiveEndDate.difference(currentWeek.startDate).inDays + 1;
    ...
    test(
      'Average calculation should return 0 when periodDays is 0 or negative',
      () {
    ...
    test(
      'Average calculation should handle future periods (period starts after today)',
      () {
        final now = DateTime.now();
        final today = DateTime(now.year, now.month, now.day, 23, 59, 59);
        ...
        // Calculate effective end date
        final effectiveEndDate = futurePeriod.endDate.isAfter(today)
            ? today
        ...
        final periodDays =
            effectiveEnd.difference(futurePeriod.startDate).inDays + 1;
